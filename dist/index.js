(function (global, factory) {
  ...
  * Copyright (c) 2019 Dmitry Semenov; */
  ...
  var PhotoSwipe = function PhotoSwipe(template, UiClass, items, options) {
    ...
    */
  	var _options = {
  		swipeRatio: 0.5, //划动占图片宽度比列
  		dbZoomRatio: 1.5, //双击缩放比例
    ...
    */
  	...
  	var publicMethods = {
  		...
  		destroy: function destroy() {
  			...
  			self.template.parentNode.removeChild(self.template);
  		},
     ...
     */
  		...
  		updateSize: function updateSize(force) {
  			...
  			_setMIN_SWIPE_DISTANCE(_viewportSize.x, _options.swipeRatio);
    ...
    */
  	...
  	var _setMIN_SWIPE_DISTANCE = function _setMIN_SWIPE_DISTANCE(w, ratio) {
  		MIN_SWIPE_DISTANCE = w * ratio;
  	};
  	...
  	_ePoint1 = {},
  	    ...
  	    _tempPointsArr = [],
  	    _tempCounter,
  	    $createTouchList = function $createTouchList(p, dec) {
  		return {
  			x: p.x + dec,
  			y: p.y + dec,
  			id: p.id + 1
  	    ...
  	    $initZoom = function $initZoom(startPointsList) {
  		if (!_isZooming && startPointsList.length > 1 && !_mainScrollAnimating && !_mainScrollShifted) {
  			_startZoomLevel = _currZoomLevel;
  			...
  			if (!_direction && !_moved && !_isZooming) {
  				if (_mainScrollPos.x !== _slideSize.x * _currPositionIndex) {
  					...
  					if (Math.abs(diff) >= DIRECTION_CHECK_OFFSET) {
  						...
  						if (_direction === 'v' && !_canPan()) {
  							touchesList[1] = $createTouchList(touchesList[0], 10);
  							$initZoom(touchesList);
  						} else {
  				...
  				if (_direction === 'v') {
  					if (self.currItem.initialPosition.y < _panOffset.y) {
  						var c = 10 + (self.currItem.initialPosition.y - _panOffset.y) / 20;
  						c = c < 0 ? 0 : c;
  					} else {
  						var c = 10;
  					}
  	...
  	_renderMovement = function _renderMovement() {
  		...
  		if (_isZooming && numPoints > 1) {
  			...
  			if (zoomLevel < minZoomLevel) {
  				if (_direction === 'v' || !_options.pinchToClose || _wasOverInitialZoom || _startZoomLevel > self.currItem.initialZoomLevel) {
     	...
     	if((!_canPan() && delta.y > 0) || _verticalDragInitiated) {
     		_currPanDist.y += delta.y;
     		_panOffset.y += delta.y;
     		_currPanDist.x += delta.x;
     		_panOffset.x += delta.x;
     			$close = delta.y > 0 && _panOffset.y > self.currItem.initialPosition.y
     		
     		var opacityRatio = _calculateVerticalDragOpacityRatio();
     		_verticalDragInitiated = true;
     		_shout('onVerticalDrag', opacityRatio);
     			if (_panOffset.y >= self.currItem.initialPosition.y) {
  	...
  	_onDragRelease = function _onDragRelease(e) {
  		...
  		if (_verticalDragInitiated) {
  			...
  			{
  				var initalPanX = _panOffset.x,
  				    initalPanY = _panOffset.y,
  				    initialBgOpacity = _bgOpacity;

  				//741
  				_animateProp('verticalDrag', 0, 1, 300, framework.easing.cubic.out, function (now) {
  					_panOffset.y = (self.currItem.initialPosition.y - initalPanY) * now + initalPanY;
  					_panOffset.x = (self.currItem.initialPosition.x - initalPanX) * now + initalPanX;
  	...
  	_initDragReleaseAnimationData = function _initDragReleaseAnimationData() {
  		...
  		if (gestureType === 'swipe') {
  			var totalShiftDist = _currPoint.x - _startPoint.x,
  			    isFastLastFlick = _releaseAnimData.lastFlickDist.x > 40;
  			// if container is shifted for more than MIN_SWIPE_DISTANCE, 
  			// and last flick gesture was in right direction
  			if (totalShiftDist > MIN_SWIPE_DISTANCE || isFastLastFlick && _releaseAnimData.lastFlickOffset.x > 40) {
  				// go to prev item
  				itemsDiff = -1;
  			} else if (totalShiftDist < -MIN_SWIPE_DISTANCE || isFastLastFlick && _releaseAnimData.lastFlickOffset.x < -40) {
  				// go to next item
  				itemsDiff = 1;
  			}
  		}
  				...
  				if (!_moved && !_isMultitouch && !_numAnimations) {
  					...
  					if (tapTimer) {
  						...
  						if (_isNearbyPoints(p0, tapReleasePoint)) {
  							...
  							if (destZoomLevel <= minZoomLevel) {
  								destZoomLevel = minZoomLevel + (maxZoomLevel - destZoomLevel) * (_options.dbZoomRatio - 1);
  							} else {
  								destZoomLevel = minZoomLevel;
  							}
  							self.zoomTo(destZoomLevel, _currPoint, 300, framework.easing.cubic.out);
  							return;
  						}
  					}
  ...
  var utils = {
    ...
    hasAttr: function hasAttr(elm, attr) {
      return elm.hasAttribute && elm.hasAttribute(attr);
    },
    hasOwn: function hasOwn(prototype, key) {
      return _hasOwn.call(prototype, key);
    },
    getTagetImgIndex: function getTagetImgIndex(imgs, targetImg) {
      var idx = 0;
    ...
    getImgItems: function getImgItems(imgs, realAddress) {
      var _this = this;

      imgs = Array.prototype.slice.apply(imgs);
      return imgs.map(function (img) {
        return _this.getImgInfos(img, realAddress);
      });
    },
    getImgWH: function getImgWH(img) {
      var result = void 0;
    ...
    getImgInfos: function getImgInfos(img, realAddress) {
      var realSrc = realAddress && this.getAttr(img, realAddress);
      var hw = this.getImgWH(img);
    ...
    isComplianceImg: function isComplianceImg(img, attr) {
      return img.tagName.toLocaleLowerCase() === 'img' && (!attr || attr && utils.hasAttr(img, attr));
    },
  ...
  var VuePhotoSwipe = function () {
    ...
    createClass(VuePhotoSwipe, [{
      ...
      value: function clickFn(event) {
        ...
        var selector = 'img' + (this.swipeAttr && '[' + this.swipeAttr + ']');
        var imgs = this.clickElm.querySelectorAll(selector);

        var items = utils.getImgItems(imgs, this.realAddress);
        var index = utils.getTagetImgIndex(imgs, target);
        ...
        utils.extend(this.options, {
          ...
          getThumbBoundsFn: function getThumbBoundsFn(index) {
            ...
            var rectTop = rect.top,
                imgH = rect.height;
            var y = rectTop + imgH < 0 ? pageYScroll - imgH : rectTop > windowH ? pageYScroll + windowH : rectTop + pageYScroll;
            return {
              x: rect.left,
              w: rect.width,
              y: y
        ...
        setTimeout(function (_) {
          _this.gallery = new PhotoSwipe(_this.pswpElm, null, items, _this.options);
          _this.gallery.init();
          _this.pswpElm.querySelector('.pswp_extends_close').addEventListener('click', _this.closeFnbind);
        });
      }
    }, {
      key: 'unbind',
